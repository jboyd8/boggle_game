...
class TestBoggle(unittest.TestCase):
    ...
    def test_grid_size_is_width_times_height(self):
        """
        Test in to ensure that the total sized of the grid
    ...
    def test_all_grid_neighbours(self):
        ...
        for pos in grid:
            others = list(grid) #creates a bew list from the dictionary's keys
            others.remove(pos)
            self.assertListEqual(sorted(neighbours[pos]), sorted(others))

    def test_coverting_a_path_to_a_word(self):
        """
        Ensure that path can be converted to words
        """
        grid = boggle.make_grid(2, 2)
    ...
    def test_load_dictionary(self):
        """
        test that the get_dictionary functions returns a dictionary
